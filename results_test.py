#!/usr/bin/env python3
"""
results_test.py — BraTS2021 Evaluation Script
"""
import os
import re
import csv
...
OUTPUT_CSV         = "detailed_brats_results.csv"
BATCH_SIZE         = 2
NUM_WORKERS        = 2
...
# Auto-search best setup on a fast subset
AUTO_FIND_BEST_SETUP = True
SEARCH_SUBSET_SIZE   = 200   # ✅ increased from 100
...
def postprocess_masks(pred_mask):
    """
    pred_mask: numpy (3, D, H, W) — channels: [ET, TC, WT]
    Enforces anatomical hierarchy: ET ⊆ TC ⊆ WT
    """
...
def predict_batch(model, images, et_thr, tc_thr, wt_thr,
    ...
    # Fallback: if ET empty but TC has signal, try lower threshold
...
def evaluate(model, dataloader, device, et_thr, tc_thr, wt_thr,
    ...
    for batch in dataloader:
        ...
        if debug_first and not debug_printed:
            ...
            lbl0 = labels[0]
            pr0  = pred_mask[0].cpu()
            print(f"  GT voxel sums (ET,TC,WT): {lbl0[0].sum():.1f} {lbl0[1].sum():.1f} {lbl0[2].sum():.1f}")
            print(f"  PR voxel sums (ET,TC,WT): {pr0[0].sum():.1f} {pr0[1].sum():.1f} {pr0[2].sum():.1f}")
            debug_printed = True
...
# THRESHOLD SEARCH
# ════════════════════════════════════════════════════════════════
def search_best_setup(model, dataloader, device):
    print("\n⚠️ Running FAST search on subset...\n")

    # ✅ expanded ET range to find better threshold for small/diffuse tumors
    et_range = [0.10, 0.12, 0.15, 0.18, 0.20, 0.22, 0.25]
    tc_range = [0.35, 0.40, 0.45]
    wt_range = [0.28, 0.32, 0.35, 0.38]

    best_dice   = -1
    best_config = {}

    print("Searching best setup on subset...")
    for et in et_range:
        for tc in tc_range:
            for wt in wt_range:
                res = evaluate(
                    model, dataloader, device,
                    et_thr=et, tc_thr=tc, wt_thr=wt,
                    use_tta=True, use_post=True, apply_sigmoid=True,
                    debug_first=False
                )
                df   = pd.DataFrame(res)
                mean = (df["Dice_ET"].mean() + df["Dice_TC"].mean() + df["Dice_WT"].mean()) / 3
                print(f"Dice={mean:.4f} | sigmoid=True tta=True post=True "
                      f"thr=({et:.2f},{tc:.2f},{wt:.2f})")
                if mean > best_dice:
                    best_dice   = mean
                    best_config = {"et": et, "tc": tc, "wt": wt}

    print(f"\nBest setup found:")
    print(f"  Dice            : {best_dice:.4f}")
    print(f"  APPLY_SIGMOID   : True")
    print(f"  USE_TTA         : True")
    print(f"  USE_POSTPROCESS : True")
    print(f"  Thresholds      : ET={best_config['et']:.2f}, "
          f"TC={best_config['tc']:.2f}, WT={best_config['wt']:.2f}")
    return best_config
...
def main():
    ...
    # ── Threshold search ─────────────────────────────────────────
    et_thr, tc_thr, wt_thr = ET_THR, TC_THR, WT_THR

    if AUTO_FIND_BEST_SETUP:
        search_indices = list(range(min(SEARCH_SUBSET_SIZE, len(subset_ds))))
        search_ds      = Subset(subset_ds, search_indices)
        search_loader  = DataLoader(search_ds, batch_size=BATCH_SIZE,
                                    shuffle=False, num_workers=NUM_WORKERS)
        best   = search_best_setup(model, search_loader, device)
        et_thr = best["et"]
        tc_thr = best["tc"]
        wt_thr = best["wt"]

    # ── Full evaluation ──────────────────────────────────────────
    print(f"\nRunning evaluation on ALL {len(subset_ds)} patients: "
          f"sigmoid={APPLY_SIGMOID}, tta={USE_TTA}, post={USE_POSTPROCESS}, "
          f"ET={et_thr:.2f}, TC={tc_thr:.2f}, WT={wt_thr:.2f}\n")
